...
//         // 中央 pawn 比邊 pawn 更有價值，避免一直只推 a/b/e pawn
//         if(advance > 0){
//             if(to_c == 2){
//                 score += 120;   // c-file
//             }else if(to_c == 1 || to_c == 3){
//                 score += 70;    // b/d-file
//             }
//         }
//     }
...
//     return score;
// }
// static int moved_piece_hanging_penalty(State* state, const Move& m) {
//     int fr = m.first.first;
//     int fc = m.first.second;
//     int tr = m.second.first;
//     int tc = m.second.second;

//     int moving = state->piece_at(state->player, fr, fc);

//     // king 不在這裡處理，king 安全用 safe_actions
//     if(moving == 0 || moving == 6){
//         return 0;
//     }

//     // pawn 中局可以交換，不要在 root 過度懲罰
//     if(moving == 1){
//         return 0;
//     }
...
//     for(const Move& reply : next->legal_actions){
//         int r2 = reply.second.first;
//         int c2 = reply.second.second;

//         if(r2 == tr && c2 == tc){
//             int ar = reply.first.first;
//             int ac = reply.first.second;

//             int attacker = next->piece_at(next->player, ar, ac);

//             if(attacker == 0){
//                 continue;
//             }

//             // int loss = val[moving] - val[attacker] / 2;
...
//     // 2 秒模式下先限制最大深度，避免深搜被不準 evaluation 帶歪
//     // if (depth > 9) {
//     //     depth = 9;
//     // }
...
// // Root safety filter:
// // 不要選「走完後對方下一手可以直接吃我 king」的 move。
...
//     bool hangs_king = (next->game_state == WIN);
...
// // 對剩下的安全 moves 排序。
// // moved_piece_hanging_penalty 每個 move 只算一次。
// std::vector<std::pair<int, Move>> scored_actions;
// scored_actions.reserve(actions.size());

// for(const Move& action : actions){
//     int s = move_score(state, action);
//     s -= moved_piece_hanging_penalty(state, action);
//     scored_actions.push_back({s, action});
// }
